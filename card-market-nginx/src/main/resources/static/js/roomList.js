$(document ).ready(function(){
    ...
    $.ajax({
    	url:"http://localhost/RoomService/rooms",
    	type:"GET",
    	success: function(data) {
        	var i;
        	var json;
    		for(i=0;i<data.length;i++) {
        		...
        		if (json.id_joueur1 != null) {
	        		$.ajax({
	        			url:"http://localhost/UserService/getName/" +json.id_joueur1,
	        			type:"GET",
	        			success: function ( data ) {
...
function onRoomSelected(id){
	...
	$.ajax({
		url: "http://localhost/RoomService/getBet/" + id,
		type:"GET",
		async: false,
		success: function(data){
			roomBet = data;
			...
			$.ajax({
				  url:"http://localhost/RoomService/addPlayer/"+ id.toString() + "/" + values.id,
				  type:"GET",
				  async: false,
				  success: function(){
					  //Withdraw money
					  $.ajax({
		    			  url:"http://localhost/UserService/removeMoney/"+ values.id + "/" + roomBet,
		    			  type:"GET"
					  });
