...
/*****************************************************************************************************************************************/
	...
	function addFocusListener(type){
		...
		$("#imgrow").on("click", ".cardchild" ,function(event){
		    ...
		    if($(event.target).closest("td").attr('class') == "cardbutton"){
		    	...
		    	$.ajax({
					  url:"http://localhost:8083/MarketService/" + type + "/" + json.id + "/" + id,
					  type:"GET",
					  success: function( data ){
						  if(data == true){
	...
	function updateMoney(){
		$.ajax({
			  url:"http://localhost:8081/UserService/money/" + json.id,
			  type:"GET",
			  success: function( data ){
					  $("#money").text(data);
			  }
	...
	function displayCraft(){
		...
		$("#CraftButton").on("click", function(){
			...
			$.ajax({
				  url:"http://localhost:8082/CardService/craft/" + json.id,
				  type:"GET",
				  success: function( data ){
					  if(data == ""){
						  toast("Not enought money", "Error", "error");
